...
cities_table = "cbs_cities"


def downgrade():
    op.drop_table(sd_involved_table) # pylint: disable=no-member
    op.drop_table(sd_accident_table) # pylint: disable=no-member
    op.drop_column(cities_table, 'population') # pylint: disable=no-member
    op.drop_column(cities_table, 'id_osm') # pylint: disable=no-member
    op.drop_column(cities_table, 'lat') # pylint: disable=no-member
    op.drop_column(cities_table, 'lon') # pylint: disable=no-member


def upgrade():
    create_safety_data_accident_table()
    create_safety_data_involved_table()
    op.add_column(cities_table, sa.Column('population', sa.Integer(), nullable=True)) # pylint: disable=no-member
    op.add_column(cities_table, sa.Column('id_osm', sa.BigInteger(), nullable=True)) # pylint: disable=no-member
    op.add_column(cities_table, sa.Column('lat', sa.Float(), nullable=True)) # pylint: disable=no-member
    op.add_column(cities_table, sa.Column('lon', sa.Float(), nullable=True)) # pylint: disable=no-member
