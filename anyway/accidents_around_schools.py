...
def main(start_date, end_date, distance, output_path):
    ...
    df_total_involved_count = (df_total.groupby(
        ['school_name', 'school_longitude', 'school_latitude', 'school_yishuv_symbol', 'school_yishuv_name',
         'anyway_link', 'school_id', ])
                               .size()
                               .reset_index(name='injured_count')
                               .sort_values('injured_count', ascending=False))
    df_total_involved_count.reset_index().to_csv(os.path.join(output_path, 'df_total_involved_count.csv'),
                                                 encoding=CONTENT_ENCODING, header=True)

    df_total_involved_by_injury = (df_total.groupby(
        ['school_id', 'school_name', 'school_longitude', 'school_latitude', 'school_yishuv_symbol',
         'school_yishuv_name', 'involved_injury_severity', 'anyway_link', ])
                                   .size()
                                   .reset_index(name='injured_count')
                                   .sort_values('injured_count', ascending=False))
    df_total_involved_by_injury.reset_index().to_csv(os.path.join(output_path, 'df_total_involved_by_injury.csv'),
                                                     encoding=CONTENT_ENCODING, header=True)

    df_total_involved_injiry_severity_1_2 = (
        df_total[(df_total.involved_injury_severity == 1) | (df_total.involved_injury_severity == 2)].groupby(
            ['school_id', 'school_name', 'anyway_link', 'school_longitude', 'school_latitude', 'school_yishuv_symbol',
             'school_yishuv_name'])
            .size()
            .reset_index(name='injured_count')
            .sort_values('injured_count', ascending=False))
    df_total_involved_injiry_severity_1_2.reset_index().to_csv(
        os.path.join(output_path, 'df_total_involved_injiry_severity_1_2.csv'), encoding=CONTENT_ENCODING, header=True)

    df_total_accident_count = (df_total.drop_duplicates(
        ['school_id', 'school_name', 'anyway_link', 'school_longitude', 'school_latitude', 'school_yishuv_symbol',
         'school_yishuv_name', 'provider_and_id'])
    ...
    df_total_accident_count.reset_index().to_csv(os.path.join(output_path, 'df_total_accident_count.csv'),
                                                 encoding=CONTENT_ENCODING, header=True)

    df_total_involved_count_by_yishuv = (df_total.groupby(
        ['school_yishuv_name', 'school_id', 'school_name', 'anyway_link_with_filters', 'school_longitude',
         'school_latitude', 'involved_injury_severity'])
                                             .size()
                                             .reset_index(name='injured_count')
                                             .loc[:, ['school_yishuv_name', 'school_name', 'anyway_link_with_filters',
                                                      'involved_injury_severity', 'injured_count', 'school_longitude',
                                                      'school_latitude', 'school_id']])
    df_total_involved_count_by_yishuv = df_total_involved_count_by_yishuv.set_index(
        ['school_yishuv_name', 'school_name', 'anyway_link_with_filters', 'school_longitude', 'school_latitude',
         'school_id', 'involved_injury_severity']).unstack(-1)
    ...
    (slice('injured_count'), slice(None))] = df_total_involved_count_by_yishuv.loc[:,
                                             (slice('injured_count'), slice(None))].apply(lambda x: x.apply(int))
    df_total_involved_count_by_yishuv['total_injured_count'] = (
        df_total_involved_count_by_yishuv.loc[:, ['injured_count']].sum(axis=1)).apply(int)

    groups = df_total_involved_count_by_yishuv.loc[:,
             ['school_yishuv_name', 'school_name', 'school_longitude', 'school_latitude', 'school_id',
              'total_injured_count']].groupby(['school_yishuv_name'])
    ...
    joined_df = pd.merge(df_total_involved_count_by_yishuv.reset_index(), rank_in_yishuv,
                         on=['school_yishuv_name', 'school_name', 'school_longitude', 'school_latitude'], how='left')
    ...
    joined_df = joined_df.loc[:,
                ['school_yishuv_name', 'school_name', 'rank_in_yishuv', 'school_longitude', 'school_latitude',
                 'injured_count_1', 'injured_count_2', 'injured_count_3', 'total_injured_count_',
                 'anyway_link_with_filters', 'school_id']]
    joined_df.columns = ['school_yishuv_name', 'school_name', 'rank_in_yishuv', 'school_longitude', 'school_latitude',
                         'killed_count', 'severly_injured_count', 'light_injured_count', 'total_injured_killed_count',
                         'anyway_link', 'school_id']
    joined_df.to_csv(os.path.join(output_path, 'df_total_involved_count_by_yishuv.csv'), encoding=CONTENT_ENCODING,
                     header=True)
