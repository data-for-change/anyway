...
class TopRoadSegmentsAccidentsPerKmWidget(SubUrbanWidget):
    ...
    def get_top_road_segments_accidents_per_km(
        ...
        accidents_table = AccidentMarkerView
        query = get_query(
            table_obj=accidents_table, filters=filters, start_time=start_time, end_time=end_time
        )

        query = (
            query.with_entities(
                accidents_table.road_segment_name,
                accidents_table.road_segment_length_km.label("segment_length"),
                cast(
                    (func.count(accidents_table.id) / accidents_table.road_segment_length_km),
                    Numeric(10, 4),
                ).label("accidents_per_km"),
                func.count(accidents_table.id).label("total_accidents"),
            )
            .filter(accidents_table.road_segment_name.isnot(None))
            .group_by(
                accidents_table.road_segment_name, accidents_table.road_segment_length_km
            )
