...
def get_injured_around_schools(start_date, end_date, distance):
    schools = db.session.query(SchoolWithDescription) \
        .filter(not_(and_(SchoolWithDescription.latitude == 0, SchoolWithDescription.longitude == 0)), \
                ...
                or_(SchoolWithDescription.school_type == 'גן ילדים',
                    SchoolWithDescription.school_type == 'בית ספר')).all()
    ...
    df_total_injured = (df_total.groupby(
        ['school_yishuv_name', 'school_id', 'school_name', 'school_type', 'school_anyway_link', 'school_longitude',
         'school_latitude', 'accident_year', 'involved_injury_severity'])
                            .size()
                            .reset_index(name='injured_count')
                            .loc[:,
                        ['school_yishuv_name', 'school_id', 'school_name', 'school_type', 'school_anyway_link',
                         'involved_injury_severity', 'injured_count', 'school_longitude', 'school_latitude',
                         'accident_year']])
    df_total_injured = df_total_injured.set_index(
        ['school_yishuv_name', 'school_id', 'school_name', 'school_type', 'school_anyway_link', 'school_longitude',
         'school_latitude', 'accident_year', 'involved_injury_severity']).unstack(-1)
    ...
    df_total_injured.loc[:, (slice('injured_count'), slice(None))] = df_total_injured.loc[:,
                                                                     (slice('injured_count'), slice(None))].apply(
    ...
    groups = (df_rank_by_yishuv.loc[:,
              ['school_yishuv_name', 'school_id', 'school_name', 'school_type', 'school_longitude', 'school_latitude',
               'total_injured_count']]
              .groupby(['school_yishuv_name']))

    df_rank_by_yishuv['rank_in_yishuv'] = groups['total_injured_count'].rank(method='dense', ascending=False).astype(
        int)
    df_rank_by_yishuv = df_rank_by_yishuv.loc[:,
                        ['school_yishuv_name', 'school_id', 'school_name', 'school_type', 'school_longitude',
                         'school_latitude', 'rank_in_yishuv']]

    # join df_total_injured and df_rank_by_yishuv with rank by yishuv
    logging.info('join df_total_injured and df_rank_by_yishuv')

    joined_df = pd.merge(df_total_injured.reset_index(), df_rank_by_yishuv,
                         on=['school_yishuv_name', 'school_id', 'school_name', 'school_type', 'school_longitude',
                             'school_latitude'], how='left')
    ...
    joined_df = joined_df.loc[:,
                ['school_yishuv_name', 'school_id', 'school_name', 'school_type', 'school_anyway_link_',
                 'rank_in_yishuv', 'school_longitude', 'school_latitude', 'accident_year_', 'injured_count_1',
                 'injured_count_2', 'injured_count_3', 'total_injured_count_']]
    joined_df.columns = ['school_yishuv_name', 'school_id', 'school_name', 'school_type', 'school_anyway_link',
                         'rank_in_yishuv', 'school_longitude', 'school_latitude', 'accident_year', 'killed_count',
                         'severly_injured_count', 'light_injured_count', 'total_injured_killed_count']
