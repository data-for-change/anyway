...
def _generate_main_data(data: pd.DataFrame, schools_ids: np.array) -> dict:
    required_columns = ["school_id", "accident_year", "killed_count",  "severly_injured_count", "light_injured_count", "total_injured_killed_count"]
    final_dict = {}
    for school_id in schools_ids:
        school_data = data[data.school_id == school_id]
        school_data = school_data.reindex(columns=required_columns)
        school_records = school_data.to_dict(orient='records')
        final_dict[str(school_id)] = school_records
    return final_dict


FILE_TO_GENERATE_METHOD = {
    SchoolsJsonFile.MAIN_FILE: _generate_main_data
}
...
if __name__ == '__main__':
    ...
    end_date=datetime(year=2021, month=1, day=1), file_suffix='noa_kirel', distance=0.5)
