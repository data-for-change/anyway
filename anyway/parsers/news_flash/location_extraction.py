...
class UrbanAddress:
    ...
    def __repr__(self):
        return "UrbanAddress(%s, %s)" % (self.city, self.street, self.street2)
...
def process_urban(streets, cities, threshold_city=70, threshold_street=50, ratio=0.85,
                   city=geo_location_dict['city'],street1='', street2=''):
    if city!='':
        ...
        for street in [street1,street2]:
            if street!='':
                ...
                if len(suspected_streets) > 0:
                    ...
                    result_yishuv.append(suspected_street_city)
        if len(result_streets)>0:
            if len(result_streets==1):
                return UrbanAddress(city=result_yishuv[0], street1=result_streets[0])
            else:
                return UrbanAddress(city=result_yishuv[0], street1=result_streets[0], street2=result_streets[1])
        else:
...
def get_db_matching_location_of_text(text, geo_location_dict=None):
    ...
    if is_urban(text, geo_dict=geo_location_dict):
        ...
        if geo_location_dict is not None:
            ...
            return process_urban(streets, cities, city=geo_location_dict['city'],street1=street1, street2=street2)
