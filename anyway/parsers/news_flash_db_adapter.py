...
class DBAdapter:
    ...
    def get_markers_for_location_extraction(self):
        query_res = self.execute('''SELECT DISTINCT road1,
                    road2,
    ...
    def get_description(self, news_flash_id):
        ...
        """
        description = self.execute('SELECT description FROM news_flash WHERE id=:id',
    ...
    def get_title(self, news_flash_id):
        ...
        """
        title = self.execute('SELECT title FROM news_flash WHERE id=:id',
                             {'id': news_flash_id}).fetchone()
        return title[0]
    ...
    def get_source(self, news_flash_id):
        ...
        """
        source = self.execute('SELECT source FROM news_flash WHERE id=:id',
                              {'id': news_flash_id}).fetchone()
        return source[0]

    def insert_new_flash_news(self, title, link, date_parsed, author, description, location, lat, lon, resolution,
        ...
        self.execute('INSERT INTO news_flash (tweet_id, title, link, date, author, description, location, lat, lon, '
    ...
    def update_news_flash_bulk(self, news_flash_id_list, params_dict_list):
        if len(news_flash_id_list) > 0 and len(news_flash_id_list) == len(params_dict_list):
            for i in range(len(news_flash_id_list)):
                self.update_news_flash_by_id(news_flash_id_list[i], params_dict_list[i], commit=False)
            self.commit()
    ...
    def get_all_news_flash_data_for_updates(self, source=None, id=None):
        if id is not None:
            ...
            res = self.execute('SELECT DISTINCT id, title, description, source, location FROM news_flash').fetchall()
        return res
