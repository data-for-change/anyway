...
def main(dest_folder, download_images):
    ...
    for link_suffix in all_road_page_links:
        ...
        for road_svg in all_road_svgs:
            ...
            road_num, road_file_name = get_road_num_from_svg_link(road_num_pattern, road_svg)
            if road_num is None:
                continue
...
def find_all_road_svg_links(link_suffix, svg_extension_pattern):
    ...
    if svg_extension_pattern.search(link_suffix) is not None:
        ...
        try:
            ...
            logger.debug(f"could not get page - {WIKIPEDIA_RELEVANT_DOMAIN + link_suffix}. skipping...")
            return []
...
def get_svg_file_from_svg_page(road_svg_link_suffix, svg_extension_pattern):
    ...
    try:
        ...
        logger.debug(f"could not get image file - {WIKIPEDIA_RELEVANT_DOMAIN + road_svg_link_suffix}. skipping...")
        return None
...
if __name__ == "__main__":
    ...
    parser.add_argument('--download_images', default=False, type=bool,
