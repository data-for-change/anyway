...
TEXT_FOR_AFTER_INFOGRAPHICS_MESSAGE = 'מקור המידע בלמ"ס. נתוני התאונה שבמבזק לא נכללים באינפוגרפיקה. ' 'הופק באמצעות ANYWAY מבית "נתון לשינוי" למידע נוסף:'
...
def fetch_message_id_for_initial_message_in_discussion_group(
    ...
    for _ in range(tries):
        ...
        for update in updates:
            if (
                update.message
                and update.message.content_type == "text"
                and update.message.forward_from_message_id
                == thread_starting_message_in_channel.message_id
            ):
...
def fetch_transcription_by_widget_name(newsflash_id):
    widgets_url = (
        f"{ANYWAY_BASE_API_URL}/infographics-data?lang=he&news_flash_id={newsflash_id}&years_ago=5"
    )
    widgets_json = requests.get(widgets_url).json()
    transcription_by_widget_name = {
        widget["name"]: widget["data"]["text"]["transcription"]
...
def send_after_infographics_message(bot, message_id_in_group, newsflash_id):
    ...
    return bot.send_message(
        TELEGRAM_LINKED_GROUP_CHAT_ID, message, reply_to_message_id=message_id_in_group
    )
...
def publish_notification(newsflash_id):
    ...
    message_id_in_group = fetch_message_id_for_initial_message_in_discussion_group(
        bot, initial_message_in_channel
    )

    if message_id_in_group:
        for infographic_name, url in urls_by_infographic_name.items():
            text = (
                transcription_by_widget_name[infographic_name]
                if infographic_name in transcription_by_widget_name
                else None
            )
            bot.send_photo(
                TELEGRAM_LINKED_GROUP_CHAT_ID,
                url,
                reply_to_message_id=message_id_in_group,
                caption=text,
            )
...
def create_public_urls_for_infographics_images(folder_name):
    ...
    objects_contents = S3_client.list_objects_v2(Bucket=INFOGRAPHICS_S3_BUCKET, Prefix=folder_name)[
        "Contents"
    ]
    presigned_urls = {}
    for object in objects_contents:
        ...
        url = S3_client.generate_presigned_url(
            "get_object", Params={"Bucket": INFOGRAPHICS_S3_BUCKET, "Key": key}
        )
