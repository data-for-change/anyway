...
from anyway import config, secrets
...
def init_flask():
    ...
    if secrets.get("PROXYFIX_X_FOR"):
        from werkzeug.middleware.proxy_fix import ProxyFix

        app.wsgi_app = ProxyFix(
            app.wsgi_app,
            x_for=int(os.environ["PROXYFIX_X_FOR"]),
            x_host=int(secrets.get_with_default("PROXYFIX_X_HOST", "0")),
            x_port=int(secrets.get_with_default("PROXYFIX_X_PORT", "0")),
            x_prefix=int(secrets.get_with_default("PROXYFIX_X_PREFIX", "0")),
            x_proto=int(secrets.get_with_default("PROXYFIX_X_PROTO", "0")),
        )
    return app
