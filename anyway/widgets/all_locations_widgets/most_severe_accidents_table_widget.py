...
def get_most_severe_accidents_table_title(
    ...
    if resolution == BE_CONST.ResolutionCategories.SUBURBAN_ROAD:
        return (
            "Most severe accidents in segment",
            segment_dictionary[location_info["road_segment_name"]],
        )
    elif resolution == BE_CONST.ResolutionCategories.SUBURBAN_JUNCTION:
        return (
            "Severe accidents in suburban junction",
            f" {location_info['non_urban_intersection_hebrew']}",
        )
    elif resolution == BE_CONST.ResolutionCategories.STREET:
        in_str = _("in")
        return (
            "Severe accidents in street",
            f" {location_info['street1_hebrew']} {in_str}{location_info['yishuv_name']}",
        )
