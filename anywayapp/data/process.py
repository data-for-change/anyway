...
FIELD_FUNCTIONS = {
	...
	"SEMEL_YISHUV" : ("סמל ישוב", number, None), #from dictionary
	"REHOV1" : ("רחוב 1", number, None), #from dicstreets (with SEMEL_YISHUV)
	"REHOV2" : ("רחוב 2", number, None), #from dicstreets (with SEMEL_YISHUV)
	"BAYIT" : ("מספר בית", number, None),
	"ZOMET_IRONI" : ("צומת עירוני", number, None),#from intersect urban dictionary
	"KVISH1" : ("כביש 1", number, None), #from intersect urban dictionary
	"KVISH2" : ("כביש 2", number, None),#from intersect urban dictionary
	"ZOMET_LO_IRONI" : ("צומת לא עירוני", number, None),#from non urban dictionary
...
def import_data():
	...
	dictionary = [x for x in csv.DictReader(open(dictionary_file))]
