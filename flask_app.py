...
def acc_in_area_query():
    # start_date and end_date are optional parameters and should be sent in DD-MM-YYYY format
    ...
    # {latN}))&start_date=<DD-MM-YYYY>&end_date=<DD-MM-YYYY>"
    ...
    if start_date is not None:
        start_date = utilities.valid_date(start_date)
    if end_date is not None:
        end_date = utilities.valid_date(end_date)
    ...
    if (start_date is not None) and (end_date is not None):
        query_obj = query_obj.filter(AccidentMarker.created >= start_date).filter(AccidentMarker.created < end_date)
