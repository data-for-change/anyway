...
class TestInvolvedQuery(unittest.TestCase):
    ...
    def test_e2e(self):
        ...
        actual = test_client.get(
            "/involved/groupby?sy=2019&ey=2019&gb=year&gb2=lca&city=5000,1&sort=d"
        )
        self.assertEqual("200 OK", actual.status, "5")
        self.assertTrue(len(actual.json) > 0, "9")
        actual = test_client.get(
            "/involved/groupby?sy=2019&ey=2019&city=5000,1&gb=vcl&lim=15&sort=d"
        )
        self.assertEqual("200 OK", actual.status, "7")
        self.assertTrue(len(actual.json) > 0, "9")
        actual = test_client.get(
            "/involved/groupby?sy=2019&ey=2019&city=5000,1&gb=cpop&sort=a"
        )
